...
<div class="form-group ${hasErrors(bean: planInstance, field: 'name', 'error')} required">
  ...
  <g:renderErrors bean="${planInstance}" field="name"/>
</div>
...
<div class="form-group ${hasErrors(bean: planInstance, field: 'period', 'error')} required">
	...
	<g:select name="period" from="${planInstance.constrainedProperties.period.inList}" required="" value="${fieldValue(bean: planInstance, field: 'period')}" valueMessagePrefix="plan.period" class="form-control"/>
</div>
